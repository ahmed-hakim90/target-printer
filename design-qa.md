...
- Catalog pass: extracted text from all 18 PDF pages and visually inspected the rendered Coming Soon and Consumables pages. Added a responsive, bilingual three-card Coming Soon section for Nail, Coffee and Portable printers; added the four catalog consumable groups to Spare Parts and catalog-listed highlights to eight production models. Tablet-width browser QA confirmed the section hierarchy, card contrast and all three release-status labels.
- Infinite-motion pass: replaced the fixed-direction marquee reset with Embla Auto Scroll, duplicated only the visual track, and verified continuous transforms in both LTR (`-1px` to `-59px`) and RTL (`290px` to `336px`) without document overflow. At 768px reviews render three cards in one row (406/496/406px high), and keyboard navigation transfers the 496px center emphasis to the next review. At 390px and 320px the carousel shows one 88%-width card plus a next-card cue with zero horizontal overflow and no stacking.
